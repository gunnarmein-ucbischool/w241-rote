...
```{r}
library(plm)
dataset = load_data_long()
head(dataset)
panel = pdata.frame(dataset, c("subject", "question"))
model = plm(score ~ treat*question,
            data = panel, model="random")
summary(model)
```

```{r}
model2 = glm(score ~ treat*question,
...
```{r}
ggplot()+
  geom_line(aes(x=question, y=score))
```
